...
export default function ClientPortfolio() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Load reactions and views for each story
      ...
      setClientPosts(postsWithInteractions);
      setLoading(false);
    }
    fetchData();
  }, [username]);
  ...
  const handleReaction = async (storyId, type) => {
    ...
    // Refresh page to show updated counts (or update state locally)
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center gap-3 mt-4">
            ...
            {client.facebook_url && <SocialIcon href={client.facebook_url}><svg viewBox="0 0 24 24" fill="currentColor" className="text-blue-700"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></SocialIcon>}
            {client.tiktok_url && <SocialIcon href={client.tiktok_url}><svg viewBox="0 0 24 24" fill="currentColor" className="text-black"><path d="M16.6 5.82s.51.5 0 0A4.278 4.278 0 0115.54 3h-3.09v12.4a2.592 2.592 0 01-2.59 2.5c-1.42 0-2.6-1.16-2.6-2.6 0-1.72 1.66-2.84 3.37-2.22V9.66c-3.45-.46-6.47 2.22-6.47 5.64 0 3.33 2.76 5.7 5.69 5.7 3.14 0 5.69-2.55 5.69-5.7V9.89a7.35 7.35 0 002.05.52V7.62c-.75-.05-1.35-.5-1.65-1.2z"/></svg></SocialIcon>}
          ...
          {clientPosts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {clientPosts.map((post) => {
                const isOwner = true; // Since we're on the owner's page, we can show view count
