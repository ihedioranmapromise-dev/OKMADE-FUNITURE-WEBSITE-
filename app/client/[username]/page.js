...
export default function ClientPortfolio() {
  ...
  useEffect(() => {
    async function fetchData() {
      // 1. Fetch public client profile via API (safe view)
      const profileRes = await fetch(`/api/public/worker/${username}`);
      if (!profileRes.ok) {
        setLoading(false);
        return;
      }
      ...
      // 2. We need the client ID to fetch projects and stories.
      // Since the view doesn't expose ID, we query the clients table
      // (RLS will allow read for logged users; for anon we'll rely on
      // the fact that RLS is not yet enabled. Once RLS is enabled,
      // this will need another API route.)
      ...
      // 3. Fetch completed (killed) projects
      ...
      // 4. Fetch stories from last 24 hours
